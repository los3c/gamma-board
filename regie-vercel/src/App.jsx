...
@media (max-width: 860px) {
  ...
  .rg-sidebar { width: 100%; flex-direction: row; align-items: center; padding: 12px 14px; overflow-x: auto; }
  .rg-brand { display: none; }
  .rg-nav { flex-direction: row; }
  .rg-side-stats { display: none; }
...
export default function RegieApp() {
  ...
  function openNewProject() { setProjModal({ id: null, title: "", description: "", status: "a_faire", priority: "normale", deadline: "", tags: [], lieu: "", horaires: "", partenaires: "", participants: "", subtasks: [] }); }
  function openEditProject(p) { setProjModal({ subtasks: [], ...p }); }
  function saveProject() {
    if (!projModal.title.trim()) return;
    if (projModal.id) {
      const old = projects.find((p) => p.id === projModal.id);
      const patch = stampCompletion(old.status, projModal.status, {});
      commitProjects(projects.map((p) => (p.id === projModal.id ? { ...projModal, ...patch } : p)));
    } else {
      const patch = stampCompletion(null, projModal.status, {});
      commitProjects([...projects, { ...projModal, ...patch, id: uid(), createdAt: todayISO() }]);
    }
    setProjModal(null);
  }
  ...
  if (!ready) {
    return (
      <div className="rg-app">
        <style>{STYLE}</style>
        <div className="rg-loading">Chargement de Régie…</div>
      </div>
    );
  }
    ...
    <div className="rg-app">
      ...
      <div className="rg-sidebar">
        ...
        <nav className="rg-nav">
          ...
          <button className={`rg-nav-item${view === "projets" ? " active" : ""}`} style={{ "--dot-soft": MODULE_SOFT.projets }} onClick={() => setView("projets")}>
          ...
          <button className={`rg-nav-item${view === "editorial" ? " active" : ""}`} style={{ "--dot-soft": MODULE_SOFT.editorial }} onClick={() => setView("editorial")}>
          ...
          <button className={`rg-nav-item${view === "notes" ? " active" : ""}`} style={{ "--dot-soft": MODULE_SOFT.notes }} onClick={() => setView("notes")}>
          ...
          <button className={`rg-nav-item${view === "vocal" ? " active" : ""}`} style={{ "--dot-soft": "rgba(255,122,69,.18)" }} onClick={() => setView("vocal")}>
            <Mic size={14} color="var(--danger)" />
            <span className="rg-nav-label">Mise à jour vocale</span>
          </button>
          <button className={`rg-nav-item${view === "compte" ? " active" : ""}`} style={{ "--dot-soft": "rgba(206,166,19,.18)" }} onClick={() => setView("compte")}>
      ...
      <div className="rg-main">
        ...
        {view === "vocal" && (
          <>
            <div className="rg-topbar">
              <div className="rg-topbar-title"><Mic size={17} color="var(--danger)" />Mise à jour vocale</div>
            ...
            <div className="rg-content">
              ...
              <div className="rg-panel">
                ...
                {voiceLog.length === 0 && <div className="rg-empty">Aucune mise à jour vocale appliquée pour l'instant.</div>}
                {voiceLog.map((v) => (
      ...
      {projModal && (
        <Modal title={projModal.id ? "Modifier le projet" : "Nouveau projet"} onClose={() => setProjModal(null)}
          footer={
            <>
              {projModal.id ? <button className="rg-btn rg-btn-ghost rg-btn-danger" onClick={() => { deleteProject(projModal.id); setProjModal(null); }}><Trash2 size={13} />Supprimer</button> : <span />}
              <button className="rg-btn rg-btn-primary" onClick={saveProject}><Check size={13} />Enregistrer</button>
            </>
          }>
          <ProjectForm draft={projModal} setDraft={setProjModal} />
        </Modal>
      )}
      ...
      {editoModal && (
        <Modal title={editoModal.id ? "Modifier le contenu" : "Nouveau contenu"} onClose={() => setEditoModal(null)}
          footer={
            <>
              {editoModal.id ? <button className="rg-btn rg-btn-ghost rg-btn-danger" onClick={() => { deleteEditorial(editoModal.id); setEditoModal(null); }}><Trash2 size={13} />Supprimer</button> : <span />}
              <button className="rg-btn rg-btn-primary" onClick={saveEditorial}><Check size={13} />Enregistrer</button>
            </>
          }>
